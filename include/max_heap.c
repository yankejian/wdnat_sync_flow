...
static struct abc *m_heap[30];        // 数据
...
static void maxheap_filterup(int start)
{
    ...
    struct abc *tmp = m_heap[c];        // 当前节点(current)的大小

    while(c > 0)
    {
        if(m_heap[p]->val >= tmp->val)
            break;
        else
        {
            m_heap[c] = m_heap[p];
 ...
 *    -1，表示失败
 */
int maxheap_insert(struct abc *a)
{
    // 如果"堆"已满，则返回
    if(m_size == m_capacity)
        return -1;
 
    m_heap[m_size] = a;        // 将"数组"插在表尾
